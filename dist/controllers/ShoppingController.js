...
const GetFoodAvailability = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const result = yield models_1.Vendor.find({ pincode: pincode, serviceAvailability: false }).sort([['rating', 'descending']]).populate("foods");
    if (result.length > 0) {
...
const GetTopRestaurants = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const pincode = req.params.pincode;
    const result = yield models_1.Vendor.find({ pincode: pincode, serviceAvailability: false }).sort([['rating', 'descending']]).limit(1);
    if (result.length > 0) {
        return res.status(200).json(result);
    }
    return res.status(400).json({ "Message": "Data Not Found" });
